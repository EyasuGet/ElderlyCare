...
export const assignNurse = async (req, res) => {
    ...
    try {
        const nurse = await Nurse.findById(nurseId); // use findById instead of findOne
        if (!nurse) {
        ...
        if (!user.assignedNurse.includes(nurse._id)) {
            user.assignedNurse.push(nurse._id);
            await user.save();
        }

        res.status(200).json({ message: "Nurse assigned successfully", user });
    } catch (error) {
